...
```
该层报文用于分发数据到对应模块，进一步进行处理，这已经不是Transport模块的作用范围。当其他模块发送数据时，需要事先组装好该层数据，因为Transport模块不关心也不知道数据的具体类型
...
其中，提供给其他模块的接口仅为普通发送和广播，转发仅有Transport模块需要
...
```
普通发送数据接口为send(destination, data)
因此只需要知道目的主机名和要发送的数据data(原数据层)即可发送

1. 封装数据报。将原数据包装成数据报，源主机为自身，目的主机为参数destination，经过的主机只有自身
...
```
路由转发的接口为_route(datagram)
只需要把从别的主机收到的数据报转发出去即可

1. 更新数据报的passed_by字段，加入自身信息。
2. 成帧。将数据报包装成帧，下一跳主机名通过调用routing_table模块的get函数获得，
   上一跳主机为自身
3. 发送。查询mapping_table，得到对应下一跳主机的物理地址，通过socket，
   用udp方式将包装好的帧发送给下一跳主机
...
2. 对每个不在visited里的邻居n：
  a. 封装数据报，其中，初次广播源主机为自身，二次广播源主机为传入的参数决定，目的地为n。
